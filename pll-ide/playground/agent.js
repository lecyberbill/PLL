...
import { logToTerminal, escHtml, makeNodeDraggable } from './ui.js';
...
To execute commands in subdirectories, use the optional 'cwd' parameter of the run_command tool instead of shell chaining or using 'cd'.

CRITICAL PROFESSIONAL RUNNING RULE:
Before starting or running any long-running or background processes (like servers, web apps, GUI applications, or watch modes), you MUST always run a compilation, verification, syntax-checking, or linting step (e.g. build, check, lint, or run unit tests) first, and verify it finishes successfully (status 0). If this check/build fails, you MUST fix the errors first. Spawning background execution commands on unverified code will hide build-time failures and make you blind to compilation or startup errors.`
        : `CRITICAL ENVIRONMENT WARNING:
